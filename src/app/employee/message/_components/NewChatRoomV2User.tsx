...
import { ChatRoomWithAllRelation, ChatRoomWithRelations, CommunityMessages } from '@/lib/types'
...
import { deleteMessage, fetchChatRoomMessages, fetchChatRoomWithUsersWhoChatted, fetchUsersWhoChatted, inspectChatRoom, inspectChatRoomEmployee, sendMessage } from '../../../../../actions/chat'
...
export const NewChatRoomV2User = ({
    chatroom,
}: Props) => {
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen pb-5">
            ...
            <main className="flex-1 flex flex-col bg-gray-100" onScroll={handleScroll}>
                <div className="border-b p-4 flex items-center justify-between">
                    {/* Set the avatar and name dynamically */}
                    <div className="flex items-center space-x-2">
                        <Avatar>
                            <AvatarImage alt={chatroom?.community.name ?? "User"} src={chatroom?.community?.displayPhoto ?? "/placeholder.svg?height=40&width=40"} />
                            <AvatarFallback>{chatroom.community?.name?.charAt(0).toUpperCase() ?? "U"}</AvatarFallback>
                        </Avatar>
                        <div>
                                ...
                                {message.communityId === chatroom.communityId ? (
                                    <div className="flex items-start space-x-2 p-4">
                                        {message.image ? (
                                            <>
                                                <Avatar>
                                                    <AvatarImage alt={message?.community?.name as string} src={message?.community?.displayPhoto as string} />
                                                    <AvatarFallback>{message?.community?.name?.charAt(0).toUpperCase()}</AvatarFallback>
                                                </Avatar>
                                                <div>
                                                    <Image
                                                        alt={message?.content}
                                            ...
                                            {message?.image ? (
                                                <>
                                                    ...
                                                    <Avatar>
                                                        ...
                                                        <AvatarFallback>
                                                            {message?.user?.name?.charAt(0).toUpperCase() as string}
                                                        </AvatarFallback>
                                                    </Avatar>
                                                </>
                                            ) : (
