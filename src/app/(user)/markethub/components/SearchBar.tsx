'use client'
import { ProductWithCommunityReviews } from '@/lib/types';
...
function SearchBar({
  ...
  allProduct: ProductWithCommunityReviews[];
}) {
                ...
                <>
                  ...
                  {filteredProduct.map((product) => (
                    <Combobox.Option
                      key={product.id}
                      className={({ active }) =>
                        `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-green text-white' : 'text-gray-900'
                        }`
                      }
                      value={product}
                    >
