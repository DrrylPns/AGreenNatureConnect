...
const fetchComments = async () => {
  try {
    await axios.get(`/api/user/post/${posts.id}/comments`)
    .then((result)=>{
    ...
    <div className={'w-full border-b border-gray-400 pb-5 shadow-sm drop-shadow-sm'}>
      {session ? (
        ...
        >
          ...
          <div className='w-full flex justify-end items-center '>
            <Button type='submit' variant={'green'} className='px-4 py-0 rounded-full text-sm'>
              Comment
            </Button>
          </div>
        </form>
      ...
      {comments ? (
        ...
        {Array.isArray(comments) && comments.map((comment: Comment) =>(
          <>
            <div className='flex w-full gap-2 items-center' key={comment.id}>
              ...
              <Popover >
                <Popover.Button><AiOutlineEllipsis/></Popover.Button>
                <Transition
                ...
                <Popover.Panel className='absolute top-0 bg-white z-30 px-2 py-1 text-sm drop-shadow-sm shadow-md rounded-lg'>
                  <button type='button' className='flex gap-1 hover:underline w-full'><AiOutlineEdit/> Edit</button>
