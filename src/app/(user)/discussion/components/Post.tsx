  ...
  return (
    <section className="sm:px-[3%] lg:pr-[30%]">
      {Posts.pages.map((post)=>(
        <div key={post.nextId}>
          {post.getAllPost !== undefined && post.getAllPost.map((post)=>(
            <Link
            href={{
          ...
          {isFetchingNextPage && <div className="transition-all text-center duration-500 ease-in-out animate-bounce">Loading...</div>}
          <span ref={ref} className="invisible absolute">intersection observer marker</span>
        </div>
      ))}
    </section>
  );
