            ...
            if (err instanceof AxiosError) {
              if (err instanceof AxiosError) {
                if (err.response?.status === 400) {
                  return toast({
                    title: "Action failed!",
                    description: "Username can't be changed again within 30 days!",
                    variant: "destructive",
                  });
                }
    ...
    return (
        <>
            {!profileState ? (
                ...
                <h2 className='cursor-pointer text-gray-400' onClick={() => setProfileState((prev) => !prev)}>Edit profile</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className='flex items-center justify-between text-[0.8rem] font-bold pl-5 my-5'>
                    <div>
                        <h3>Age</h3>
                        <p className='text-[0.8rem] text-gray-400  font-normal'>{isBirthdayNull ? "You did not set your age yet."
                            : `render age here based on birthday`}
                        </p>
                    </div>
                </div>
                <div className='flex items-center justify-between text-[0.8rem] font-bold pl-5 my-5'>
