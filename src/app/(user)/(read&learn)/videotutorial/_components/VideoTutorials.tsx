...
export const VideoTutorials = () => {
    const { isLoading, isError, data: videoTutorials } = useQuery({
        queryKey: ['blogs'],
        queryFn: async () => {
            try {
    ...
    if (videoTutorials.length === 0) return <div className="flex flex-col items-center">
        <Image
            alt="No result found."
            className="w-96 h-96"
            ...
            {videoTutorials.map((video) => (
                <div className="grid grid-cols-2 grid-rows-1 grid-flow-row gap-2 space-x-7">
                            ...
                            <CardFooter className="absolute bg-black/40 bottom-0 z-10 border-t-1 border-default-600 dark:border-default-100 shadow-md">
                                <div className="flex flex-grow gap-2 items-center">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <p className="text-sm text-white/60 underline">{video.community.name} {" "} Community</p>
                                    </div>
                                </div>
                            </CardFooter>
                        </Card>
                    ...
                    <div className="">
                        ...
                        <p>By {video.author.name} {" "} {video.author.lastName}</p>
