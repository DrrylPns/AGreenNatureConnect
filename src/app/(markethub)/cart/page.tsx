...
function CartPage() {
  ...
  const calculateSubtotal = (selectedItems: Cart[]) => {
    return selectedItems.reduce((total, item) => {
      const priceToAdd = item.variant.product.isFree
        ? 0
        ...
        <div className="sm:px-3 pb-32">
          {cartItems.length < 1 && (
            <div className="w-full flex flex-col gap-5 justify-center h-[50vh] items-center">
              <h1 className="text-center text-2xl text-gray-400 font-medium font-livvic ">
                Your cart is currently empty right now!
              </h1>
              <Link
                href={"/markethub"}
                className="px-5 py-2 border-2 rounded-md border-green bg-transparent hover:bg-yellow-400  font-poppins font-medium transition-colors duration-500 ease-in-out mx-auto"
              >
                Shop now
              </Link>
            </div>
          ...
          {Object.entries(groupedItems).map(
            ([communityName, communityItems]) => (
              <div
                key={communityName}
                className="mb-2 pb-2 bg-gray-50 shadow-sm drop-shadow-md"
              >
                <div className="flex items-center gap-20 border-y border-black py-2 px-10">
                  <h2
                    id={`selectAll_${communityName}`}
                    className="text-[0.6rem] sm:text-[1rem] font-poppins font-bold"
                  >
                  ...
                  >
                    ...
                    <div>
                      <input
                        ...
                        disabled={
                          (item.variant.unitOfMeasurement === "Kilograms" &&
                            item.variant.product.kilograms <
                              item.variant.variant) ||
                          (item.variant.unitOfMeasurement === "Grams" &&
                            item.variant.product.grams <
                              item.variant.variant) ||
                          (item.variant.unitOfMeasurement === "Pounds" &&
                            item.variant.product.pounds <
                              item.variant.variant) ||
                          (item.variant.unitOfMeasurement === "Packs" &&
                            item.variant.product.packs <
                              item.variant.variant) ||
                          (item.variant.unitOfMeasurement === "Pieces" &&
                            item.variant.product.pieces < item.variant.variant)
                        }
                      />
                    </div>
                    <Image
                    ...
                    <div className="ml-auto">
                      <h3 className="text-[12px] sm:text-sm ">
                        {" "}
