        ...
        {!loading && item && (
          <>
            {item.selectedProduct.community.qrCode === null ? (
              <h1>
                GCach payment for Community{" "}
                {item.selectedProduct.community.name} is not Available right
                now!
              </h1>
            ) : (
              <div className="bg-gray-200 md:p-10 shadow-md drop-shadow-md p-5">
                <div className="w-full bg-white text-center">
                  ...
                  <h1 className="font-semibold text-center md:text-3xl">
                    {item.selectedProduct.community.name}
                  </h1>
                </div>
                ...
                <div className="mt-5">
                  <p className="text-muted-foreground text-base">
                    Note: if you can't scan the code, the number of community is
                    available
                  </p>
                  <h1>{item.selectedProduct.community.contactNumber}</h1>
                </div>
              </div>
            )}
          </>
