...
export function RatingStars({ readonly, average, width, onChange }: RatingStarsProps) {
  ...
  return (
    <div className="flex flex-row items-center">
      <ReactRating style={{ maxWidth: width, }} itemStyles={myStyles} value={average as number} onChange={onChange} readOnly={readonly} halfFillMode='svg' />
      <h1 className='ml-4'>{average}</h1>
    </div>
  )
}